...
const Transactions = () => {
  ...
  const renderSplitBetween = useCallback((_, { splitBetween }) => splitBetween.map((username) => <Button key={username}>{username}</Button>), []);
  ...
  return (
    <Table
      ...
      columns={Object.values(EXPENSE_DETAILS).map((col) => ({
        ...
        dataIndex: col,
        key: col,
        render: col === EXPENSE_DETAILS.SPLIT_BETWEEN ? renderSplitBetween : null,
      }))}
